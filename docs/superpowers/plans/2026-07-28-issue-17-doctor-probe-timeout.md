...
- [x] Add the normalized schema property to `policy.required` with integer min/max; historical
  raw configs may omit it because `migrateConfig()` supplies the default before validation.
...
| AC5 | Normalized schema requires integer [1000, 300000]; migration supplies omissions before validation | Task 2 | `schemas/config.schema.json` | `test/schema.test.ts` | Focused schema tests | Schema requires the normalized field, accepts valid bound values, and rejects out-of-range/wrong-type |
