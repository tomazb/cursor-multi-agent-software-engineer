...
- Duplicate delivery: return success without repeating side effects. Only `completed` deliveries are terminal; stale `processing` claims (crash mid-handler) become reclaimable after a TTL so GitHub retries are not stuck as permanent duplicates. Each claim carries a lease nonce; `complete` durably stages the completed record before moving the processing record aside, installs with `wx` (never POSIX rename-over a successor), and recovers staging/reclaim artifacts on the next read/claim so a crash cannot leave an empty ledger open to redispatch. Mismatch restore uses `wx` only — never `rename` onto an occupied path. Rejected completion surfaces as a handler error (not HTTP 200). Stale reclaims and owner-mismatch attempts are counted for monitoring (`diagnostics()` / callbacks).
...
- Concurrent check creates for the same idempotency key are serialized; create/association locks reclaim only when owner death is proven via `ESRCH` (EPERM and other probe errors fail closed as alive). Malformed locks are never reclaimed. Reclaim uses rename-aside + wx-only restore (never rename-over) and fails closed on unsupported filesystem operations (lock acquire times out). Release is identity-bound.
